...
class App:
    def __init__(self, username=config['user']['username'],
                 password=config['user']['password'],
                 target_username=config['user']['target_username'],
        ...
        if self.error is False:
            if not os.path.exists(path):
                os.mkdir(path)  # create a directory if doesn't exist
    ...
    def close_dialog_box(self):
        ...
        try:
            sleep(randint(1, 2))
            close_dialog = self.driver.find_element_by_xpath(
